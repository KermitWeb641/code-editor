...
function initSortable() {
    Sortable.create(elements.fileTabsContainer, {
        animation: 150,
        ghostClass: 'sortable-ghost',
        onEnd: () => {
            const newOrderIds = Array.from(elements.fileTabsContainer.children).map(el => el.dataset.id);
            const newFiles = [];
...
function renderTabs() {
    // Clear existing tabs in the container
    elements.fileTabsContainer.innerHTML = '';
    ...
    files.forEach(file => {
        ...
        elements.fileTabsContainer.appendChild(btn);
    });
}
